...
for current_day in range(1, remaining_days + 1):
    ...
    if current_day % 2 == 0:
        total_cost += quantity_of_decors * ornament_price
        total_spirit += 5
    if current_day % 3 == 0:
        total_cost += quantity_of_decors * (tree_skirt_price + tree_garland_price)
        total_spirit += 3 + 10
    if current_day % 5 == 0:
        total_cost += quantity_of_decors * tree_lights_price
        total_spirit += 17
        if current_day % 3 == 0:
            total_spirit += 30
    if current_day % 10 == 0:
